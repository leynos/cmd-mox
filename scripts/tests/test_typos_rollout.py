...
def test_committed_local_policy_preserves_api_literal_exemption(
    rollout_modules: tuple[types.ModuleType, types.ModuleType, types.ModuleType],
    tmp_path: Path,
) -> None:
    """The generated configuration retains the reviewed API literal exemption."""
    ...
    assert "`color`" in config["default"]["extend-ignore-re"], (
        "The committed literal external API spelling exemption must survive "
